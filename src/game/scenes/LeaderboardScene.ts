...
export class LeaderboardScene extends BaseScene {
    ...
    private async loadLeaderboard(): Promise<void> {
        const loadingText = this.add
            .text(GAME_WIDTH / 2, 250, 'Загрузка...', {
                fontFamily: 'Arial',
                fontSize: '24px',
                color: '#ffffff',
            })
        ...
        try {
            ...
            const leaderboard = await this.yaSDK
                .getLeaderboards()
    ...
    private displayLeaderboard(): void {
        if (this.leaderboardEntries.length === 0) {
            this.add
                .text(GAME_WIDTH / 2, 250, 'Нет данных для отображения', {
                    fontFamily: 'Arial',
                    fontSize: '24px',
                    color: '#ffffff',
                })
    ...
    private displayError(): void {
        this.add
            .text(GAME_WIDTH / 2, 250, `Ошибка загрузки: ${this.errorMessage}`, {
                fontFamily: 'Arial',
                fontSize: '20px',
                color: '#e74c3c',
            })
        ...
        this.createTextButton(GAME_WIDTH / 2, 320, 'Повторить', () => {
